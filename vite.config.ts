      ...
      async closeBundle() {
        ...
        if (existsSync('dist/content.css')) {
          ...
          if (contentCss.length > 100) {
            ...
            console.log('Found popup styles in content.css, will move to popup.css');
          }
        ...
        for (const cssFile of cssFiles) {
          if (cssFile !== 'content.css' && cssFile !== 'popup.css') {
            ...
            if (fileContent.length > 100) {
              ...
              unlinkSync(filePath);
              console.log(`Found popup styles in ${cssFile}, moving to popup.css`);
            }
          }
        }
        ...
        if (popupCssContent) {
          ...
          writeFileSync('dist/popup.css', '/* Popup styles should be here */\n');
          console.warn('Warning: No popup styles found!');
        }
        
        // Create empty content.css file (for content script - should be empty)
        writeFileSync('dist/content.css', '/* Empty - no styles injected into web pages */\n');
        ...
        if (existsSync('dist/index.html')) {
          ...
          htmlContent = htmlContent.replace(/href=["'][^"']*\.css["']/g, 'href="/popup.css"');
